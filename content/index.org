...
Although it is possible to [[https://nixos.org/manual/nixos/stable/index.html#module-services-emacs][configure Emacs with Nix]],
I prefer to use the native configuration. The latter is version controlled with git and can be [[https://github.com/Panadestein/nixos-config][automatically]] set up with =home-manager= like this: 
  ...
  (use-package org-tempo
    ...
    (setq org-todo-keywords
      '((sequence "TODO(t)" "BROKEN(b)" "FEEDBACK(f)" "VERIFY(v)" "URGENT(u)"
         "|" "DONE(d)" "OPTIONAL(o)" "DELEGATED(d)" "IRRELEVAT(i)")))
...
#+begin_src emacs-lisp
  ...
  ;; Thanks to Jimmy Aguilar Mena (Ergus) for this suggestio
