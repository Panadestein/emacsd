...
with =org-mode= using Emacs in batch mode. You can take a look at my =build-site.el=
script in the source [[https://github.com/Panadestein/emacsd][repo]] to see how I achieve
this in practice.
...
version of some packages (like =org-mode=). While there are tools like =Quelpa= that allow this without
ditching =package.el=, I decided to go for the option that ensures reproducibility: [[https://github.com/radian-software/straight.el][straight.el]]. With this
...
slowly start to /klappt/, and for some major modes it just does not make sense to use Evil.
...
I prefer to use pop-ups for the translation, so I need to install the =posframe= package in addition.
The translation directions can be changed in the prompt with =C-n= and =C-p=.
...
** Interoperability with Nyxt

The Emacs of the web browsers is =Nyxt=. With this little package we can improve the communication between the two programs:
...
like to try my configuration, feel free to [[https://github.com/Panadestein/emacsd/fork][fork it]] on GitHub. So that's it, let's gracefully finish tangling
everything:
